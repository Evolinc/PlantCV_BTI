...
def main():
    ...
    #img1 = pcv.white_balance(img, roi=(xcoord1,ycoord1,100,100))
    img1 = img
    img1 = pcv.rotate(img=img1, rotation_deg=, crop=False)
    ...
    roi_contour, roi_hierarchy = pcv.roi.rectangle(img=img1, x=xcoord2, y=ycoord2, h=1900, w=1900)
    ...
    rois1, roi_hierarchy1 = pcv.roi.multi(img=img1, coord=(xcoord3,ycoord3), radius=VALUE, spacing=(475, 475), nrows=4, ncols=4)
    ...
    for i in range(0, len(rois1)):
        ...
        if test.dtype == 'int32':
            ...
            if test2.dtype == 'uint8':
                ...
                pcv.outputs.add_observation(variable = 'plantID', trait = 'roi', method = 'roi', scale = 'int', datatype = int, value= filename[:-4] + "_" + str(i), label = '#')
